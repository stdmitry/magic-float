...
(function () {
	...
	fabric.Viewport = (function () {
		...
		Viewport.prototype.translate = function () {
			var x = (this.position.x + this.grabPointer.x - this.grabStartPointer.x) / this.zoom;
			var y = (this.position.y + this.grabPointer.y - this.grabStartPointer.y) / this.zoom;
			return {
				x: x, //Math.min(0, x),
				y: y  //Math.min(0, y)
			};
	...
	fabric.CanvasWithViewport = (function (_super) {
		...
		function CanvasWithViewport() {
			...
			this.onRenderBackground = false;
			CanvasWithViewport.__super__.constructor.apply(this, arguments);
		}

		CanvasWithViewport.prototype.beginDraw = function (ctx) {
			ctx.save();
			ctx.scale(this.viewport.zoom, this.viewport.zoom);
			ctx.translate(this.viewport.translate().x, this.viewport.translate().y);
		};

		CanvasWithViewport.prototype.endDraw = function(ctx) {
			return ctx.restore();
		};

		CanvasWithViewport.prototype.getZoom = function (zoom) {
		...
		CanvasWithViewport.prototype.__onMouseUp = function (e) {
			if (this.isGrabMode && this._isCurrentlyGrabbing) {
